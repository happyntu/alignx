...
- [x] Query benchmark: AXF vs BAM on region / coverage / pileup / filter workloads
  ...
  - AXF1 view: **2.3x-2.9x faster** than samtools on all regions after parallel chunk decode (8 threads, sliding window `std::async`); 85 ms vs 247 ms (chr1:1M-2M), 61 ms vs 175 ms (chrY:20M-21M), 1,669 ms vs 3,878 ms (centromeric)
