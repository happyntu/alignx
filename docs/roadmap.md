...
- [x] Reference-delta SEQ codec (Item 6): `seq_ref_delta` (codec ID 13) stores only mismatches, soft-clips, and insertions as delta from reference FASTA. CIGAR-driven reconstruction at decode time. AXF1 v3 typed extension block for reference identity (per-contig SHA-256 checksums). Self-contained SHA-256, FASTA reader (HTSlib faidx), reference validator with caching. Gated behind `--reference <fasta>` CLI opt-in. Per-chunk fallback to `seq_2bit_literal` when ref-delta is not smaller. See ADR-007. HG002 PacBio chr1:1M-2M: 7/7 SEQ chunks use `seq_ref_delta`; raw AXF1 **-20.7%** (40.0 MB vs 50.5 MB), zstd AXF1 **-15.5%** (27.6 MB vs 32.7 MB) vs 2-bit literal baseline. SAM stdout SHA-verified identical to samtools view.
- [x] Python bindings (pybind11): `_alignx` C extension module via `ALIGNX_BUILD_PYTHON=ON`. Binds all enums (ColumnId, CodecId, Compression, QualityLossy), structs (Record, Reference, Chunk, ChunkIndex, FileMetadata, FileIndex, WriteOptions, RecordFilter, CoverageResult), Axf1FileReader (open, query_chunks, read_chunk, read_chunk_columns), high-level functions (view, coverage, convert, export_bam), and HTSlib-gated BamReader/FastaReader with Python iterator + context manager patterns. numpy/pandas integration via `alignx` wrapper package: `CoverageResult.depth` as zero-copy numpy array (buffer protocol), `Chunk.column_array()` for fast numeric extraction, `Chunk.to_dataframe()` and `CoverageResult.to_series()` for pandas, `query_region()` region→DataFrame pipeline, `bam_to_dataframe()`. 57 pytest tests.
- [x] Python packaging: `pyproject.toml` with scikit-build-core backend enables `pip install .` from source. `_alignx.pyi` type stubs cover all C++ extension API (enums, classes, functions, HTSlib-gated types) for IDE autocompletion and mypy. PEP 561 `py.typed` marker for automatic type discovery. `__all__` re-export list in `alignx/__init__.py`. Wheel layout: `_alignx.so` + `_alignx.pyi` at root, `alignx/` package.
- [x] Multi-threaded AXF1 encoding: `write_axf1_file()` parallel chunk encoding via atomic-counter thread pool (up to 8 workers, threshold >= 16 chunks). `write_chunk()` is a pure function with no shared mutable state — each worker encodes assigned chunks independently. Sequential fallback for small files. Python bindings release GIL during CPU-intensive operations (`view`, `coverage`, `convert`, `export_bam`). **Benchmark result**: HG002 PacBio chr1:1M-2M (324 chunks, zstd): sequential median 44,049 ms vs parallel 47,313 ms — **parallel is ~7% slower** because BAM reading (HTSlib, sequential) dominates total convert time (>90%); thread creation + cache contention overhead exceeds encoding parallelism benefit at this scale. `ALIGNX_SEQUENTIAL_ENCODE=1` env var forces sequential path for A/B comparison. Parallel encoding may benefit future streaming-write pipelines where BAM I/O is decoupled from encoding.
