...
- AXF1 view decode optimization batch 2: per-chunk streaming write, pointer-based bulk SEQ/QUAL decode, pointer-based varint reader, bulk chunk I/O (single read per chunk for ≥5 columns), Reader class refactored to pointer+size. Result: AXF1 view 1.06x-1.16x faster on 1 Mb, 0.70x on centromeric.
- AXF1 view parallel chunk decode: sliding-window `std::async` (up to 8 threads, ≥16 chunks threshold). Main thread reads bytes sequentially; workers decode+filter+format in parallel.
...
- Multi-threaded AXF1 encoding: `write_axf1_file()` uses atomic-counter thread pool (up to 8 workers) for parallel chunk encoding when >= 16 chunks; `write_chunk()` is a pure function (const chunk → encoded bytes) with no shared mutable state; sequential fallback for small files; `ALIGNX_SEQUENTIAL_ENCODE=1` env var forces sequential path for benchmarking. **Benchmark**: HG002 chr1:1M-2M (324 chunks, zstd) sequential 44s vs parallel 47s — parallel ~7% slower because BAM reading dominates; encoding parallelism benefit deferred to streaming-write pipeline. Python bindings release GIL during `view()`, `coverage()`, `convert()`, `export_bam()` for concurrent Python thread execution
