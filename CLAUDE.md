...
- AXF1 view decode optimization batch 2: per-chunk streaming write, pointer-based bulk SEQ/QUAL decode, pointer-based varint reader, bulk chunk I/O (single read per chunk for ≥5 columns), Reader class refactored to pointer+size
- AXF1 view parallel chunk decode: sliding-window `std::async` (up to 8 threads, ≥16 chunks threshold). Main thread reads bytes sequentially; workers decode+filter+format in parallel. Result: AXF1 view **2.3x-2.9x faster than samtools** (85 ms vs 247 ms chr1:1M-2M, 61 ms vs 175 ms chrY:20M-21M, 1,669 ms vs 3,878 ms centromeric)
