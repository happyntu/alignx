...
class SummarizeAxf1ColumnsTest(unittest.TestCase):
    ...
    def test_summarizes_payload_bytes_and_codec_distribution(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("2\tflag\t1\t1\t2:flag_bitpack:1\t1.000\t12.500", summary)
            self.assertIn("5\tcigar\t1\t1\t5:cigar_token:1\t1.000\t12.500", summary)
            self.assertIn("9\tsequence\t1\t1\t4:seq_2bit_literal:1\t1.000\t12.500", summary)
            self.assertIn(
                "10\tquality\t2\t2\t7:qual_pack:1,8:qual_pack_compressed:1\t1.000\t25.000",
                summary,
            )
            self.assertIn("99\tunknown_99\t1\t1\t77:unknown_77:1\t1.000\t12.500", summary)
